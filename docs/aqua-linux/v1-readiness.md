...
explicit rescan, WPA2-Personal credential entry, disconnect, root-owned
saved-credential reconnect, and saved-network forget. Scan input is capped at
4096 bytes and 32 rows, deduplicated by SSID,
and reduced to four typed results; the broker response remains capped at 512
bytes. Settings renders at most two results beside explicit rescan and forget
actions, blocks unsupported security, and
keeps the 8-63-byte printable-ASCII passphrase in a masked, redacted, fixed-size
buffer that is wiped on cancel or after every submission. A failed association
permits one explicit retry before the two-attempt entry flow closes. The opt-in Wi-Fi profile
...
lifecycle, explicit rescan, saved-credential reconnect, metadata-validated
forget, and rejection of reconnect after forget. This is virtual-radio
evidence, not physical-radio evidence; default network ownership, WPA3
association, and physical support remain gated. The
