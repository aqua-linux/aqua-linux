...
| Network adapter | Validated | The opt-in QEMU profiles prove virtio-net DHCP and bounded recovery plus mac80211_hwsim broker discovery, WPA2 credential association, DHCP, disconnect and saved-credential reconnect, service recovery, and cleanup. Settings enables discovery and bounded WPA2 controls only while that broker is present; default activation remains disabled. | `scripts/check-network-qemu.sh` and `scripts/check-wifi-qemu.sh` |
...
| Wi-Fi | Validated | Two mac80211_hwsim radios prove bounded discovery of the isolated access point, new WPA2-Personal credential association, and Settings-compatible saved-credential reconnect. This is virtual-radio evidence only. | `scripts/check-wifi-qemu.sh` |
