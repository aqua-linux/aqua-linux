...
| Network adapter | Validated | The opt-in QEMU profiles prove virtio-net DHCP and bounded recovery plus mac80211_hwsim broker discovery and explicit rescan, WPA2 credential association, DHCP, disconnect and saved-credential reconnect, service recovery, saved-network forget, rejection of reconnect after forget, and cleanup. Settings enables discovery and bounded WPA2 controls only while that broker is present; default activation remains disabled. | `scripts/check-network-qemu.sh` and `scripts/check-wifi-qemu.sh` |
...
| Wi-Fi | Validated | Two mac80211_hwsim radios prove bounded discovery and explicit rescan of the isolated access point, new WPA2-Personal credential association, Settings-compatible saved-credential reconnect, safe forget, and rejected reconnect after forget. This is virtual-radio evidence only. | `scripts/check-wifi-qemu.sh` |
| Bluetooth | Not tested | No virtual Bluetooth workflow is claimed. | None |
