...
| Network adapter | Validated | The opt-in QEMU profiles prove virtio-net DHCP and bounded recovery plus mac80211_hwsim WPA2 association, DHCP, broker-authenticated disconnect and saved-credential reconnect, service recovery, and cleanup. Settings enables only the Wi-Fi association control while that broker is present; default activation remains disabled. | `scripts/check-network-qemu.sh` and `scripts/check-wifi-qemu.sh` |
...
| Wi-Fi | Validated | Two mac80211_hwsim radios prove the gated WPA2-Personal lifecycle and Settings-compatible saved-credential reconnect. This is virtual-radio evidence only. | `scripts/check-wifi-qemu.sh` |
