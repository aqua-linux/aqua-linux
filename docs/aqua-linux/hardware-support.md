...
| Network adapter | Validated | The opt-in QEMU profile proves virtio-net DHCP, a default route, external DNS lookup, typed renewal through the authenticated broker, root-client rejection, route-loss recovery, forced DHCP-client recovery, post-recovery broker availability, and continuing recovery-shell access. Settings controls and default activation remain disabled. | `scripts/check-network-qemu.sh` |
