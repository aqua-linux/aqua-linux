...
- The longer `qemu-tcg-bochs-qualification-v1` runner requires at least 900,000 ms and fifteen real launcher open/dismiss cycles. Every state change waits for its own DRM repaint acknowledgement, preventing queued input from being mistaken for presented interaction. It requires at least 15 input samples and 45 keyboard events, zero crashes/drops, the established frame/input/memory limits, a duration-scaled 2,160,000,000 us CPU ceiling, full cleanup, recovery, and diagnostic isolation. The first accepted run observed 1,344,436 ms, 94 keyboard events, 71 frames, 38 input samples, 8,131 us maximum page-flip wait, 44,045,721 us maximum input latency, 1,232,394,556 us CPU time, and 134,200 KiB RSS growth. This remains non-physical QEMU evidence and explicitly reports `release_ready=false`; repeated qualification is still open.
