...
- Aqua Core Icon loading accepts only the 13 reviewed project-authored SVGs and rejects active content, text, filters, gradients, embedded raster data, and external references before parsing. The pure-Rust resvg path runs without default resource features and rasterizes each icon at the requested physical size instead of enlarging bitmaps. A 256-entry LRU includes source revision, role, theme, interaction state, logical size, and output scale in every key. The committed 1,456-case fixture matrix covers all roles, themes, required sizes, and supported scales; packaged QEMU integration remains open.
