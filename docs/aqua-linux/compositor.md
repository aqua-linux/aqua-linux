...
- Renderer-independent R2 presentation acceptance types for bounded idle, window-interaction, animation, and multi-client samples. The report rejects production full-frame readback and CPU copies, mismatched frame/page-flip accounting, missing callbacks or damage, settled-idle repaint timers, absent or over-budget latency/resource measurements, excess dropped frames, and diagnostic readback that touches production frames. Deterministic host fixtures validate the evaluator only; the report cannot authorize a release claim and does not replace live QEMU or physical measurements.
