...
- Applications cards and desktop Files, Settings, and Trash items now derive their outer bounds, icon and label slots, selection state, keyboard activation, accessibility semantics, and pointer targets from one shared grid-cell contract. Gaps between Applications cards no longer activate a neighboring item. Files retains its real list-row view because no runtime file grid mode exists yet; its list arrows, Page Up/Down, Home, and End now use the shared bounded list target and reveal-offset contract while visible text preview keeps separate key ownership. The aggregate four-theme packaged-QEMU component run covers this shared path.
