...
- Renderer-independent R2 presentation acceptance types cover bounded idle, window-interaction, animation, and multi-client samples. An ordered telemetry collector turns frame requests, page flips or drops, callbacks, damage, latency, readback, CPU-copy, idle, and resource events into immutable samples while rejecting missing requests, incomplete accounting, and counters above 100,000. The report rejects production full-frame readback and CPU copies, mismatched frame/page-flip accounting, settled-idle repaint timers, absent or over-budget latency/resource measurements, excess dropped frames, and diagnostic readback that touches production frames, uses a non-diagnostic path, or activates KMS/display output. Window interaction and shell animation require real input and multiple presented frames; only the multi-client workload requires client frame callbacks and damage commits, because shell-only repaints do not manufacture Wayland client events. The live DRM-Wayland loop records initial and repaint KMS requests before submission, measures submit-to-page-flip completion at the real event boundary, keeps native GBM/KMS and virtio fallback paths distinct, and counts dumb-buffer copies and full-frame readbacks. Aggregate Smithay damage and frame-callback counters use one observation baseline and monotonic deltas at DRM event boundaries, with a final post-flip synchronization so the last callbacks are not lost. For idle, quiet live dispatch intervals establish settled state and any later causeless repaint or active motion timer is a violation. Monotonic workload duration, process CPU-clock time, and maximum bounded Linux `VmRSS` growth complete the live resource bridge. The snapshot remains partial and reports `r2_presentation_acceptance_complete=false`; the fixed QEMU budget cannot satisfy physical or release acceptance.
...
- The packaged R2 workload runner defines four isolated recovery-returning sessions and one offscreen diagnostic probe in a single snapshot-mode QEMU boot: quiet idle, real-input shell window interaction, frame-driven shell animation plus a real input sample, packaged Files plus Settings multi-client activity, then a deterministic two-frame GLES readback with no production session or KMS output. Shell-only workloads may legitimately emit no client callback or client damage; the multi-client workload must emit both. The bounded session ceiling is 120 seconds so slow TCG software rendering can still complete the 90-second multi-client window. The runner refuses a rootfs older than the compositor source.
- The repeated R2 runner requires three through ten independent, fully validated QEMU boots and writes each run into a new evidence directory that it refuses to overwrite. Its bounded review mode rejects duplicate log paths, revalidates all four workloads and diagnostic isolation in every log, and reports per-workload observed maxima for frame time, input-to-present latency, CPU time, and memory growth. Three independent packaged boots completed on 2026-08-30 with 12 workload records and three isolated diagnostic records. Their overall maxima were 22,681 us page-flip wait, 46,522,278 us input-to-present latency, 147,346,897 us CPU time, and 130,476 KiB peak RSS growth. The reviewed `qemu-tcg-bochs-v1` regression profile enforces respective limits of 50,000 us, 60,000,000 us, 180,000,000 us, and 163,840 KiB plus zero dropped frames. Both single-run and repeated review validators enforce this fixed profile and explicitly retain `r2_physical_budget_selected=false`. The packaged kernel enables Bochs DRM and the macOS runner defaults to QEMU `bochs-display`; this exercises Aqua's `production-gbm-kms` path with llvmpipe rendering into direct GBM dma-buf scanout. The virtio target remains the explicitly identified `legacy-cpu-copy` fallback, and QEMU TCG timings are not physical responsiveness evidence.
- The bounded R2 soak runner keeps one production compositor process active for at least five minutes with Files and Settings mapped, sends ten real virtio-input cycles, and requires at least five distinct input-to-present samples after repaint coalescing. Its separate `qemu-tcg-bochs-soak-v1` profile keeps the standard timing, memory, and zero-drop limits, raises only the cumulative two-vCPU CPU ceiling to 720,000,000 us, and fixes the crash budget at zero. The first packaged run observed 338,040 ms, 42 dispatched keyboard events, nine presented frames, five input samples, 9,879 us maximum page-flip wait, 39,054,481 us maximum input latency, 128,457,628 us CPU time, and 130,252 KiB peak RSS growth with zero production readbacks or CPU copies. Graceful stop, client reap, CRTC restoration, GBM release, recovery return, and diagnostic isolation passed. Raw logs and reports live in an overwrite-protected ignored evidence directory. This is initial QEMU regression soak evidence, not physical or release-qualification stability evidence.
- The longer `qemu-tcg-bochs-qualification-v1` runner requires at least 900,000 ms and fifteen real launcher open/dismiss cycles. Every state change waits for its own DRM repaint acknowledgement, preventing queued input from being mistaken for presented interaction. It requires at least 15 input samples and 45 keyboard events, zero crashes/drops, the established frame/input/memory limits, a duration-scaled 2,160,000,000 us CPU ceiling, full cleanup, recovery, and diagnostic isolation. A bounded repeated runner accepts only three through ten fresh boots, refuses to overwrite evidence, rejects duplicate log paths, and revalidates every run. Three independent cold boots completed with three isolated diagnostic records, 213 total frames, minimum 1,319,699 ms observation, minimum 38 input samples, and minimum 94 keyboard events. Their overall maxima were 8,505 us page-flip wait, 44,045,721 us input latency, 1,232,394,556 us CPU time, and 134,348 KiB RSS growth. This closes repeated QEMU qualification while explicitly retaining `physical_evidence=false` and `release_ready=false`.
...
- Standard clipboard and primary-selection ownership are initialized through
  Smithay's `wl_data_device_manager` and
  `zwp_primary_selection_device_manager_v1` states. Selection focus follows
  Aqua Seat keyboard focus, so an unfocused client cannot replace either
  ...
  `wlr-data-control` or external data-control global is exposed.
- Standard `wl_data_device_manager` version 3 drag-and-drop uses Smithay's
  implicit pointer-grab validation and focus routing. A second bounded Linux
  probe uses two independent Wayland connections to reject a start without a
  grab, bind a valid source to the pressed origin, route enter and drop only to
  the pointer-focused target without changing keyboard focus, negotiate
  `text/plain;charset=utf-8` plus the `copy` action, and transfer an exact
  28-byte payload through the protocol file descriptor. The target never
  accepts the advertised unsupported type, reads are capped at 4096 bytes and
  two seconds, and the compositor does not buffer the payload. The accepted
  drop reports performed and finished to the source; a rejected second drop
  reports cancellation and is never delivered to the target. No privileged
  data-control global is exposed.
- Standard `zwp_text_input_manager_v3` follows Aqua Seat keyboard focus, while
  `zwp_input_method_manager_v2` is filtered to an explicitly authorized client
  and is invisible to both normal clients. A bounded three-client Linux probe
  rejects an unfocused enable, activates only the focused text input, forwards
  surrounding text, content type, change cause, and cursor geometry, and
  validates the synchronized input-method serial. The authorized method sends
  Turkish UTF-8 `İ` preedit and `İstanbul` commit strings plus a bounded
  surrounding-text deletion only to the active client. Focus handoff leaves
  and deactivates the old client, enters and activates the new one, and blocks
  a stale request from the old client. The same probe binds the input-method
  popup to its focused parent and repositions it from the cursor rectangle.
...
- Client-window elevation uses shared control, panel, dialog, and active-window tokens. The renderer rasterizes bounded rounded shadow masks and caches them by physical geometry, output scale, theme, and elevation; the Smithay GLES path reuses uploaded mask textures, places each shadow immediately below its client surface, and computes viewport-clipped shadow damage. A committed fixture matrix covers both themes and supported scales. One recovery-safe packaged QEMU boot maps two fixture windows with exactly one focused surface, captures both themes as distinct 1280x800 PNGs, verifies two mask uploads and bounded damage regions per session, and returns to recovery after each capture.
- Aqua Core Icon loading accepts only the 13 reviewed project-authored SVGs and rejects active content, text, filters, gradients, embedded raster data, and external references before parsing. The pure-Rust resvg path runs without default resource features and rasterizes each icon at the requested physical size instead of enlarging bitmaps. A 256-entry LRU includes source revision, role, theme, interaction state, logical size, and output scale in every key. The live compositor uses cached Aqua Core rasters in its floating top panels, desktop, and notification texture; bottom controls use code-native symbols and dots. The committed host matrix covers all roles, themes, required sizes, and supported scales. A dedicated packaged QEMU scenario captures both themes and proves seven parsed roles, seven physical raster keys, zero cache hits on a forced bottom-shell refresh, zero eviction, and recovery return after every bounded session.
...
- The Linux and packaged-rootfs Wayland output probe keeps two clients connected while expanding from one to four `wl_output` globals. It verifies exact current/preferred modes, xdg-output coordinates and logical sizes, declared scales 1.0, 1.25, 1.5, and 2.0, integer fallbacks, normal/90/180/270 degree transforms, fractional-scale 150/120 delivery, committed viewporter state, and fourth-output removal while the original remains usable. This is a software protocol matrix and does not claim physical connector hotplug evidence.
- A separate Linux and packaged-rootfs surface-role probe uses two independent Wayland connections for a real `xdg_popup` and a real `wl_subsurface`. Popup parentage, exact 32,48 240x120 initial geometry, exact 64,72 repositioned geometry, two configure acknowledgements, reposition token 77, subsurface position 24,36, synchronized/desynchronized commits, child destruction, and surviving independent parents are all required with `host_stub=false`.
...
- The shared `aqua-text` service shapes renderer text with Rustybuzz before
  ...
  captures Light and Dark as two distinct
...
- The graphical QEMU acceptance run captures Light and Dark frames with Files and Settings open. It requires the Shell broadcast and both client redraw markers, unchanged application PIDs, an increased compositor repaint sequence, and a visible pixel delta before accepting the live switch.
...
- The redesigned desktop keeps the diagnostic system overview hidden while its bounded telemetry continues to refresh; the session menu still promotes the shared shell region explicitly.
...
- Desktop icon context actions now derive renderer rows, visible keyboard selection, and global pointer targets from one bounded shared menu geometry. While a desktop menu is open, the compositor owns Up, Down, Home, End, Enter, Space, and Escape and prevents every key press and release from reaching an underlying client; changing rows clears Trash's armed repeat-activation confirmation. Session actions use the same menu row and gap contract while retaining keyboard navigation and second-Enter confirmation. Menu padding and inter-row gaps do not activate actions. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Settings and Properties now derive bordered section surfaces, headings, rows, trailing controls, and footer actions from one shared section-group contract. Insets and inter-row gaps remain non-interactive, group focus does not move child geometry, and each nested switch, segmented control, or action retains its own semantics. Settings Appearance additionally routes compositor-owned Left/Right through the shared theme segmented-control Previous/Next targets, including wrap and persistence. Enter activates Reduced Motion, Desktop Icons, Key Repeat, Wi-Fi, or Audio Mute only through the active shared switch gate; Wi-Fi credential entry and authoritative disabled states retain ownership. The compact Network row now bounds its switch to the rendered 32-pixel height, keeping pointer and keyboard activation on the same valid target. Network Left/Right routes Rescan/Forget only through the shared standard-button keyboard gates, so credential entry and missing broker authority remain fail closed. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Settings category Up, Down, Home, and End keys now resolve through the same shared sidebar row count and selected target used by pointer and renderer geometry. Previous and Next wrap within the six real categories, Home and End select the bounds, invalid row sets fail closed, and active Wi-Fi credential entry consumes all four keys without changing category focus.
- Properties and System Overview now render their real Location, Items, Host, Kernel, Uptime, Load, and Memory values through one shared metadata-row contract. Label/value columns remain bounded and fitted, the rows expose read-only definition semantics, and they never absorb pointer or keyboard input from neighboring actions. The aggregate two-theme packaged-QEMU component run covers this shared path.
- The top system area now derives three floating panels, brand, calendar/clock/notification controls, Network/Audio/Battery slots, and trailing Settings target from one shared contract. The software and cached-icon render paths consume those slots, while compositor pointer routing activates only the exact Settings rectangle and rejects the adjacent status area. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Applications cards and desktop Files, Settings, and Trash items now derive their outer bounds, icon and label slots, selection state, keyboard activation, accessibility semantics, and pointer targets from one shared grid-cell contract. Gaps between Applications cards no longer activate a neighboring item. Files retains its real list-row view because no runtime file grid mode exists yet; its list arrows, Page Up/Down, Home, and End use the shared bounded list target and reveal-offset contract while visible text preview keeps separate key ownership. Files Enter additionally passes the selected visible content row, built at the active client-buffer width, through the shared list-row keyboard gate before confined folder navigation or read-only text preview; hidden selections stay inert. Left/Right transfers focus between content and a separate sidebar cursor, whose Up/Down/Home/End movement uses the shared sidebar target without navigating until Enter; pointer input clears keyboard focus. A blocked sidebar destination restores the prior active location and content selection, preventing renderer state from claiming an unopened folder. Sidebar rendering, navigation, and Enter now consume the same shared row at the active client height, and a compact layout that cannot contain all five locations rejects hidden focus and activation. Content rendering, pointer and hover routing, scrolling, scrollbar geometry, keyboard reveal, and Enter likewise share the actual-height row count, so the status bar cannot overlap an active list target. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Applications mode now composes its panel surface, title, shared search field, three-column grid, visible-item limit, and child grid cells through one application-overview contract. Renderer and pointer routing consume the same panel and cell rectangles; panel insets and grid gaps reject application activation. Up/Down and Left/Right wrap through the overview's bounded Previous/Next targets at the active output viewport, while Home/End select its visible limits; invalid compact layouts leave selection unchanged. Enter builds the selected shared grid cell from that viewport and must pass its keyboard gate before the unchanged allowlisted request is queued. Global Search stays separate for its real split results/actions layout. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Global Search mode now composes its panel, title, focused search field, split divider, result rows, and three real quick actions through one shared contract. Renderer and pointer routing consume identical result/action rectangles; headings, padding, divider space, and row gaps reject activation. Search result keyboard selection uses the active viewport composition's separate bounded Previous, Next, Home, and End targets without changing quick-action pointer behavior or text entry; invalid compact layouts leave selection unchanged. Enter builds the selected shared list row from that viewport and must pass its keyboard gate, so invalid compact compositions and empty or out-of-visible-range results cannot queue a request. The input/scene acceptance probe routes open, search, activation, and dismissal through its supplied viewport and independently proves a compact invalid Search composition emits no launch request. The aggregate two-theme packaged-QEMU component run covers this shared path.
- The reusable running-app dock primitive retains its item, icon, and indicator contracts for bounded consumers, while the desktop shell no longer composes it. Applications and Search own desktop launch discovery.
- The bottom-right workspace switcher now derives its three dot targets and selected fill from one shared contract. Renderer and compositor pointer routing consume identical half-open target geometry. Ctrl+Alt+Left/Right/Home/End resolves through the same bounded keyboard target contract, while Shift moves the active window to that destination; focus transfer and window ownership remain unchanged. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Notification toasts now derive their icon, title, body, source, dismiss target, and dismiss glyph from one shared contract. Placeholder and cached-icon renderer paths consume the same slots, while compositor pointer routing uses the exact half-open dismiss rectangle and Escape dismisses the active toast without changing the existing bounded queue, timeout promotion, visibility, or motion behavior. The toast exposes polite live-status semantics plus an independently named dismiss button. The aggregate two-theme packaged-QEMU component run covers this shared path.
- Confirmation presentation now separates shared geometry and user intent from workflow authorization. Session and Empty Trash consume compact repeat-activation prompts, while the Installer Summary real-mode panel consumes detailed exact-text geometry and confirmed state. Escape/Enter/Space map only to cancel or confirm intent, presentation padding has no implicit action, and existing Session action checks, Trash confinement, installer disk identity revalidation, exact target-bound phrase matching, QEMU/operator opt-ins, and transaction execution gates remain authoritative. The aggregate two-theme packaged-QEMU component run covers this shared path.
