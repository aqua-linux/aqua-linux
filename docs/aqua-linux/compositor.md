...
- The centered running-app dock now derives its surface, three item targets, placeholder and production icon slots, and running indicators from one shared contract. Cached and deterministic renderer paths plus compositor pointer routing consume the same geometry; transparent space around the group and its right edge do not launch applications. The aggregate two-theme packaged-QEMU component run covers this shared path.
