...
- Settings now exposes an Audio category backed by a bounded 0–100 persistent output-volume preference and mute state. The shared slider and switch consume identical renderer/input geometry, while a real, non-symlink `/dev/snd` directory controls availability. The runtime explicitly reports that no audio backend has applied these preferences; this is not playback, routing, mixer, or physical-hardware evidence.
