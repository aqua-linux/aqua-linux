...
- Real Linux Smithay `Aqua Seat` probe with a `wl_seat` global, keyboard and pointer capabilities, launcher toggle interception, pointer dispatch, Settings category selection, and launcher scene/render-plan visibility. The same feature-enabled musl binary, `libxkbcommon`, and XKB rules are packaged in the Buildroot rootfs and executed by the rootfs contract check with `host_stub=false`.
...
- The compositor-owned dock has a transparent RGBA content layer over its system-surface surface. Aqua toggles the launcher; Files, Settings, and Trash route through allowlisted launch requests; Files and Settings reflect supervised process ownership with running indicators. Repeated activation raises an existing XDG surface instead of spawning a duplicate. Dock hit testing scales the canonical 800x600 shell geometry to the active output while retaining the established 1536x1024 client input space. QEMU verifies two bounded dock launcher toggles before running the complete first-party lifecycle suite.
