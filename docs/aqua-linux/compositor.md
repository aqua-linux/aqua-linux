...
- The renderer provides shared window-chrome geometry and title-bar primitives. Installer remains on the LightWhite palette, while Files, Settings, Terminal, and Properties load the persisted LightWhite, Softtouch, Deepside, or Nightmare palette when launched. LightWhite remains the fallback for legacy or invalid settings. Terminal content keeps a dark readable monospace scrim under every frame palette. Applying a changed theme to already-running windows and to the top bar, Applications, Search, bottom shell, desktop overlays, and Installer remains later work. Client redraws are frame-coalesced so input bursts do not create one expensive QEMU GPU repaint per key.
