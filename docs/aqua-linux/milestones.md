...
- An acceptance-only presentation rehearsal now compiles the confirmed model into the canonical 20-step graph, proves the non-executing runner reports `executed=false`, renders graph-bound 40%, 65%, and 95% Installation states, reaches Completed only at 20/20 and 100%, and captures both final live Wayland surfaces. The full packaged path records 140 forwarded key events, visible-content Down/Up/End/Home Keyboard selection through `trq`, `trf`, and `us` plus Tab footer handoff, proves the intermediate `us` selection leaves the applied layout at `trq`, real virtio pointer footer hover/press, background focus blur, a rejected non-primary Language press, primary press drag cancellation, release-inside full-row Language selection, Summary background blur with rejected Enter, pointer-restored Summary focus and target-bound destructive acknowledgement, pointer-focused exact-text confirmation, a Space-triggered bounded Finish request after Completed, 72 client rerenders, and seven distinct QEMU screendumps without dispatching disk commands or restarting.
...
- Choice, disk, user-field, Summary acknowledgement, and exact-text confirmation controls share exact responsive rectangles between `aqua-renderer` and installer pointer hit-testing. They arm only on primary press, render pressed feedback, and activate only on release over the armed target; drag-out, pointer leave, and non-primary input cannot commit. The Summary text field renders entered confirmation and takes its active-control focus only after a valid release. Bounded catalog pointer activation updates validated selections and reports `ValueApplied`, blocked disks reject hits, eligible disks remain unapplied until explicit activation, and field activation cannot create a user profile.
- User Information and Summary form input is gated by visible Step Content keyboard focus. Summary joins the form focus order, background blur prevents hidden Enter or text mutation, and pointer selection restores the same focus path before editing resumes.
- Language, Keyboard, Time Zone, and Partitions consume Up, Down, Home, and End through their bounded form navigation while visible Step Content focus is active. Choice navigation changes only the selected row until Enter or Space commits the catalog-validated value; pointer release over an armed choice remains direct activation. Disk navigation skips blocked targets, Tab performs the footer focus handoff, and text-entry controls retain Space as printable input.
