...
- Choice, disk, user-field, and enabled Summary acknowledgement controls share exact responsive rectangles between `aqua-renderer` and installer pointer hit-testing. They arm only on primary press, render pressed feedback, and activate only on release over the armed target; drag-out, pointer leave, and non-primary input cannot commit. Bounded catalog activation updates validated selections, blocked disks reject hits, eligible disks remain unapplied until explicit activation, and field activation cannot create a user profile.
