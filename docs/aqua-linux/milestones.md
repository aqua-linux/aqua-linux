...
- Language, Keyboard, Time Zone, and Partitions consume Up, Down, Home, and End through their bounded form navigation while visible Step Content focus is active. Choice navigation changes only the selected row until Activate commits the catalog-validated value; pointer release over an armed choice remains direct activation. Disk navigation skips blocked targets, and Tab performs the footer focus handoff.
