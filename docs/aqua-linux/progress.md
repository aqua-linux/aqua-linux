...
Milestone 9 remains complete and the packaged installer performance path is validated. Runtime convergence now includes persisted and live-refreshed LightWhite, Softtouch, Deepside, and Nightmare palettes across Files, Settings, Terminal, Properties, all seven Installer setup screens, the compact top bar, Applications, Global Search, the three-group bottom shell, desktop overlays, system overview, session menu, and notifications. Full workspace window assignment and QEMU user-visible theme-switch evidence remain ahead without changing the Buildroot or custom Wayland compositor architecture.
...
| 2026-08-28 | M11: Polish and Public Readiness | Early | 45% | The v1 desktop and installer contracts are documented. The runtime has a compact top bar, reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, three bottom shell groups, shared first-party window chrome, and persisted plus bounded live-refreshed LightWhite, Softtouch, Deepside, and Nightmare palettes across current application, shell, and all seven Installer setup screens. Full workspace management, QEMU user-visible theme-switch evidence, contributor guide, current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Validate a user-visible live theme switch in QEMU, then implement full workspace window assignment and movement.
2. Keep fallback TTY, boot_graphics=false, and autostart=false in the default development image.
