...
Milestone 9 remains complete and the packaged installer performance path is validated. Public hardware status now separates automated QEMU evidence from unvalidated drivers, deferred workflows, and the entirely untested MSI Sword 17 target. Physical installation and daily-use claims remain prohibited until named-device validation exists.
...
| 2026-08-28 | M11: Polish and Public Readiness | In Progress | 85% | The v1 desktop and installer contracts are documented. The runtime has a compact top bar, reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, three bottom shell groups, shared first-party window chrome, four live-refreshed themes, and three real workspaces. A dedicated QEMU session publishes validated current captures with a provenance manifest. The public hardware matrix separates proven QEMU behavior, present but unvalidated drivers, deferred workflows, and wholly untested MSI Sword 17 hardware without advancing Milestone 10. Final icon clearance and issue workflow remain incomplete. |
...
1. Keep fallback TTY, boot_graphics=false, and autostart=false in the default development image.
2. Define a read-only MSI Sword 17 hardware inventory record before any physical boot or installation claim.
