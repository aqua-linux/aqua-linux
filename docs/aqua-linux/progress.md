...
Milestone 9 remains complete and the packaged installer performance path is validated. Runtime convergence now includes persisted LightWhite, Softtouch, Deepside, and Nightmare palettes across Files, Settings, Terminal, Properties, the compact top bar, Applications, Global Search, the three-group bottom shell, desktop overlays, system overview, session menu, and notifications. Full workspace window assignment, live theme broadcast, and Installer theming remain ahead without changing the Buildroot or custom Wayland compositor architecture.
...
| 2026-08-28 | M11: Polish and Public Readiness | Early | 35% | The v1 desktop and installer contracts are documented. The runtime has a compact top bar, reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, three bottom shell groups, shared first-party window chrome, and persisted LightWhite, Softtouch, Deepside, and Nightmare palettes across current application and shell surfaces. Full workspace management, live theme updates for running sessions and windows, Installer theme application, contributor guide, current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Apply the persisted theme selection to Installer, then add a compositor theme-change channel that refreshes the running shell and first-party windows without a session restart.
