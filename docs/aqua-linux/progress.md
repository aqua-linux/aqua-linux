...
Milestone 12 now has accepted typography, elevation, Aqua Core Icon integration, semantic state motion, and nineteen shared component primitives. The independent aqua-components crate owns renderer-neutral top-system-bar, window-frame, menu, metadata-row, section-group, standard-button, icon-button, search-field, switch, segmented-control, toolbar, list-row, grid-cell, application-overview, global-search, running-app-dock, workspace-switcher, notification, and sidebar-navigation contracts. Applications, Global Search, both bottom-shell groups, and notification toasts now compose their distinct surfaces, content, and pointer paths through shared geometry. Deterministic theme/viewport fixtures are accepted, while packaged-QEMU component acceptance and 3 catalog entries stay open. Checkbox and slider remain planned until real installer-option and audio-volume models exist. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 99% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The independent aqua-components crate now owns renderer-neutral top-system-bar, window-frame, menu, metadata-row, section-group, standard-button, icon-button, search-field, switch, segmented-control, toolbar, list-row, grid-cell, application-overview, global-search, running-app-dock, workspace-switcher, notification, and sidebar-navigation anatomy, stable state geometry, input activation, and accessibility semantics. Applications and Global Search retain separate shared composition contracts. The centered running-app dock shares launch targets and running indicators; the bottom-right workspace switcher shares its targets and active state; notification rendering and compositor dismissal share content slots, exact pointer geometry, Escape behavior, and polite live-status semantics without changing the bounded queue or timeout model. All nineteen primitives have deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024. Packaged-QEMU component acceptance and 3 of 22 catalog entries remain open. Checkbox and slider remain planned until real installer-option and audio-volume models exist. |
...
1. Extract the shared confirmation-dialog contract from real destructive and session confirmation models without weakening execution gates.
