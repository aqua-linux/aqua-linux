...
Milestone 12 now has accepted typography, elevation, Aqua Core Icon integration, semantic state motion, and the first shared component extraction. The standard-button primitive owns stable geometry, ten applicable states, pointer and keyboard activation, accessibility semantics, and deterministic theme/viewport fixtures; the installer footer consumes it instead of an installer-only drawing helper. Packaged-QEMU component acceptance and the remaining catalog entries stay open. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 81% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The live compositor shares one 256-entry complete-key icon LRU across four shell textures. Launcher, session-menu, and notification transitions use published duration and cubic-Bezier tokens with bounded scheduling and reduced-motion behavior. The component catalog now inventories 22 required primitives. Its standard button owns stable geometry, ten applicable states, pointer and keyboard activation, accessibility semantics, and deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024; the installer footer consumes it. Packaged-QEMU component acceptance and 21 catalog entries remain open. |
...
1. Extract shared list-row and sidebar-navigation primitives for Files, Settings, Search, and installer consumers, then extend the deterministic component matrix.
