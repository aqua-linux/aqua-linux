...
Milestone 9 remains complete and the packaged installer performance path is validated. Runtime convergence now includes the compact top bar, pale-wave wallpaper, distinct Applications and Global Search surfaces, a three-group bottom shell, and persisted LightWhite, Softtouch, Deepside, and Nightmare palettes across Files, Settings, Terminal, and Properties. Full workspace window assignment, live theme broadcast, shell theming, and Installer theming remain ahead without changing the Buildroot or custom Wayland compositor architecture.
...
| 2026-08-28 | M11: Polish and Public Readiness | Early | 30% | The v1 desktop and installer contracts are documented. The runtime has a compact top bar, reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, three bottom shell groups, shared first-party window chrome, and persisted LightWhite, Softtouch, Deepside, and Nightmare palettes across Files, Settings, Terminal, and Properties. Full workspace management, live updates for running windows, shell and Installer theme application, contributor guide, current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Apply the persisted theme selection to the top bar, Applications, Search, bottom shell, desktop overlays, and Installer, then refresh already-running first-party windows.
