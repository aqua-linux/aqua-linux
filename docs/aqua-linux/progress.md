...
Milestone 12 now has accepted typography, elevation, Aqua Core Icon integration, semantic state motion, and three shared component primitives. The independent aqua-components crate owns renderer-neutral standard-button, list-row, and sidebar-navigation anatomy, state, input, and accessibility contracts. Settings and Files consume one sidebar geometry for both shell hit testing and renderer drawing; installer steps consume the same list-row composition. Deterministic theme/viewport fixtures are accepted, while packaged-QEMU component acceptance and 19 catalog entries stay open. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 83% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The independent aqua-components crate now owns renderer-neutral standard-button, list-row, and sidebar-navigation anatomy, stable state geometry, pointer and keyboard activation, and accessibility semantics. Settings and Files use shared sidebar geometry for shell hit testing and renderer drawing; the installer uses shared buttons and step rows. All three primitives have deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024. Packaged-QEMU component acceptance and 19 of 22 catalog entries remain open. |
...
1. Extract shared search-field and icon-button primitives, migrate their shell and first-party consumers, and extend the deterministic component matrix.
