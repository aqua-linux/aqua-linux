...
Milestone 12 now has accepted typography, elevation, Aqua Core Icon integration, semantic state motion, and seven shared component primitives. The independent aqua-components crate owns renderer-neutral standard-button, icon-button, search-field, switch, segmented-control, list-row, and sidebar-navigation anatomy, state, input, and accessibility contracts. Settings consumes shared toggle and theme-selector geometry; Applications and Global Search consume the search field; Files consumes back/forward icon buttons and shares sidebar geometry with Settings; installer steps consume the list-row composition. Deterministic theme/viewport fixtures are accepted, while packaged-QEMU component acceptance and 15 catalog entries stay open. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 87% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The independent aqua-components crate now owns renderer-neutral standard-button, icon-button, search-field, switch, segmented-control, list-row, and sidebar-navigation anatomy, stable state geometry, pointer and keyboard activation, and accessibility semantics. Settings uses shared switch and theme-selector geometry; Applications and Global Search use the search field; Files uses back/forward icon buttons and shares sidebar geometry with Settings; the installer uses shared buttons and step rows. All seven primitives have deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024. Packaged-QEMU component acceptance and 15 of 22 catalog entries remain open. |
...
1. Extract shared checkbox and slider primitives with real installer and audio consumers, then extend the deterministic component matrix.
