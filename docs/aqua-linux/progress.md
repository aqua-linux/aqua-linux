...
Milestone 12 now has shared typography and tokenized elevation foundations. Four elevation levels produce reusable rounded shadow masks keyed by geometry, scale, theme, and level; the live Smithay GLES path places focused and inactive window shadows with viewport-bounded damage. A committed 16-case matrix covers every theme and supported scale, and the Linux musl cross-build validates the target-only integration. Packaged QEMU elevation capture remains before runtime acceptance. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 35% | The shared aqua-text crate owns shaped Unicode layout, deterministic fallback, six roles, four output scales, and bounded glyph caching, with committed renderer fixtures and packaged QEMU evidence. Tokenized elevation now defines control, panel, dialog, and active-window shadows; reusable rounded masks are keyed by physical geometry, scale, theme, and level, while themes alter opacity without changing geometry. The Smithay GLES path uploads and reuses those masks beneath focused and inactive client surfaces and computes viewport-clipped damage. A committed 16-case matrix covers every theme and scale, and the target-only path passes the Linux musl cross-build. Packaged QEMU elevation captures remain open, followed by icons, motion, and the component catalog. |
...
1. Capture and validate tokenized client-window elevation through the packaged Smithay/DRM QEMU path.
