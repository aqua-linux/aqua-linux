...
Milestone 9 remains complete and the packaged installer performance path is validated. Runtime convergence now includes the compositor-owned compact top bar, reproducible pale-wave wallpaper, and distinct Applications and Global Search surfaces with active-output pointer geometry and real Aqua app actions. Bottom shell groups, application chrome, installer presentation, and four-theme selection remain ahead without changing the Buildroot or custom Wayland compositor architecture.
...
| 2026-08-28 | M11: Polish and Public Readiness | Early | 15% | The v1 desktop and installer contracts are documented. The runtime now has a dedicated compact top bar, a bright reproducible pale-wave default wallpaper, and separate centered Applications and Global Search surfaces backed by the real app catalog. Remaining bottom shell controls, application chrome convergence, contributor guide, current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Rebuild the bottom shell controls as Applications/Search, centered pinned/running apps, and workspace thumbnails.
2. Unify bright window chrome and controls across Files, Settings, Terminal, Properties, and Installer.
3. Apply the four theme token sets to runtime shell and first-party window surfaces.
4. Keep fallback TTY, boot_graphics=false, and autostart=false in the default development image.
