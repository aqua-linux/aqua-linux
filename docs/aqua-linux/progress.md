...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 98% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The independent aqua-components crate now owns renderer-neutral top-system-bar, window-frame, menu, metadata-row, section-group, standard-button, icon-button, search-field, switch, segmented-control, toolbar, list-row, grid-cell, application-overview, global-search, running-app-dock, workspace-switcher, and sidebar-navigation anatomy, stable state geometry, input activation, and accessibility semantics. Applications and Global Search retain separate shared composition contracts. The centered running-app dock shares launch targets and running indicators; the bottom-right workspace switcher shares its three targets, thumbnails, active indicator, tab semantics, and exact pointer routing while preserving window ownership and focus behavior. All eighteen primitives have deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024. Packaged-QEMU component acceptance and 4 of 22 catalog entries remain open. Checkbox and slider remain planned until real installer-option and audio-volume models exist. |
...
1. Extract shared notification and confirmation-dialog contracts from their real shell models without weakening timeout or destructive confirmation gates.
