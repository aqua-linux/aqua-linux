...
| 2026-08-28 | M11: Polish and Public Readiness | Early | 20% | The v1 desktop and installer contracts are documented. The runtime now has a compact top bar, a bright reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, and three separated bottom shell groups for commands, apps, and workspace selection. Application chrome convergence, full workspace management, contributor guide, current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Unify bright window chrome and controls across Files, Settings, Terminal, Properties, and Installer.
2. Apply the four theme token sets to runtime shell and first-party window surfaces.
3. Keep fallback TTY, boot_graphics=false, and autostart=false in the default development image.
