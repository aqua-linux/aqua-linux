...
Milestone 12 now has accepted typography, elevation, Aqua Core Icon integration, semantic state motion, and twenty shared component primitives. The independent aqua-components crate owns renderer-neutral top-system-bar, window-frame, menu, metadata-row, section-group, standard-button, icon-button, search-field, switch, segmented-control, toolbar, list-row, grid-cell, application-overview, global-search, running-app-dock, workspace-switcher, notification, confirmation-dialog, and sidebar-navigation contracts. Applications, Global Search, both bottom-shell groups, notifications, and confirmation prompts compose their distinct surfaces, content, and input paths through shared geometry. Deterministic theme/viewport fixtures and a packaged acceptance-only Wayland client now prove all twenty shared primitives through the real Smithay, GLES, and DRM path in all four themes, with bounded cleanup and recovery return. Checkbox and slider remain the 2 open catalog entries until real installer-option and audio-volume models exist. Physical MSI Sword 17 validation remains unauthorized until read-only evidence is reviewed.
...
1. Define real installer-option and audio-volume models before extracting the remaining checkbox and slider primitives; do not add mock-only catalog entries.
