...
Milestone 9 remains complete and the packaged installer performance path is validated. QEMU now proves a user-visible LightWhite-to-Deepside transition across the shell, Files, and Settings without restarting either application; the captured frames differ across 37.30% of pixels. Full workspace window assignment and movement remain ahead without changing the Buildroot or custom Wayland compositor architecture.
...
| 2026-08-28 | M11: Polish and Public Readiness | In Progress | 55% | The v1 desktop and installer contracts are documented. The runtime has a compact top bar, reproducible pale-wave wallpaper, separate Applications and Global Search surfaces, three bottom shell groups, shared first-party window chrome, and persisted plus bounded live-refreshed LightWhite, Softtouch, Deepside, and Nightmare palettes across current application, shell, and all seven Installer setup screens. Dedicated QEMU acceptance now changes the shell, Files, and Settings from LightWhite to Deepside without process restarts and requires a visible frame delta. Full workspace management, broader current public OS screenshots, hardware status, final icon clearance, and issue workflow are incomplete. |
...
1. Implement full workspace window assignment and movement with keyboard, pointer, and QEMU acceptance coverage.
