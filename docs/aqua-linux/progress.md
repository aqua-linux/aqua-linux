...
| --- | --- | --- | ---: | --- |
| 2026-08-29 | M12: Visual Fidelity and Component System | In Progress | 96% | The shared aqua-text crate, tokenized elevation path, Aqua Core Icon pipeline, and semantic motion system have deterministic host fixtures plus packaged QEMU evidence. The independent aqua-components crate now owns renderer-neutral top-system-bar, window-frame, menu, metadata-row, section-group, standard-button, icon-button, search-field, switch, segmented-control, toolbar, list-row, grid-cell, application-overview, global-search, and sidebar-navigation anatomy, stable state geometry, input activation, and accessibility semantics. Applications composes its grid through one overview contract; Global Search composes its focused search field, split results, three real quick actions, and exact pointer routing through a separate contract. Panel headings, insets, dividers, and row gaps reject activation. All sixteen primitives have deterministic four-theme acceptance at 800x600, 1280x800, and fractional-scale 1536x1024. Packaged-QEMU component acceptance and 6 of 22 catalog entries remain open. Checkbox and slider remain planned until real installer-option and audio-volume models exist. |
...
1. Extract shared running-app-dock and workspace-switcher contracts from their real bottom-shell models without changing launch or workspace behavior.
