...
pub const QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET: PresentationBudget = PresentationBudget {
    max_frame_time_us: 50_000,
    max_input_to_present_us: 60_000_000,
    max_dropped_frames: 0,
...
mod tests {
    ...
    #[test]
    fn qemu_tcg_bochs_qualification_v1_budget_is_bounded_and_not_a_release_claim() {
        assert!(QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.is_bounded());
        assert_eq!(
            QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.max_frame_time_us,
            50_000
        );
        assert_eq!(
            QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.max_input_to_present_us,
            60_000_000
        );
        assert_eq!(QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.max_dropped_frames, 0);
        assert_eq!(
            QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.max_cpu_time_us,
            2_160_000_000
        );
        assert_eq!(
            QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET.max_memory_growth_kib,
            163_840
        );
        let report = R2PresentationReport::evaluate(
            PresentationEvidenceTarget::HostFixture,
            QEMU_TCG_BOCHS_QUALIFICATION_V1_BUDGET,
            &samples(),
            diagnostic(),
        );
        assert!(report.is_baseline_ready());
