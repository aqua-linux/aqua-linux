...
pub const FILES_SIDEBAR_ITEM_COUNT: usize = 5;
...
const FILES_LIST_ROW_WIDTH_INSET: u32 = 204;
const FILES_LIST_ROW_MIN_WIDTH: u32 = 71;
const FILES_CONTENT_MIN_VIEWPORT_WIDTH: u32 = FILES_LIST_ROW_WIDTH_INSET + FILES_LIST_ROW_MIN_WIDTH;
...
const FILES_PREVIEW_LINE_Y: u32 = 194;
...
pub const fn files_sidebar_navigation_in_viewport(
    width: u32,
    height: u32,
) -> Option<SidebarNavigation<'static>> {
    ...
    if !navigation.is_valid()
        || navigation.rect.right() > width
        || navigation.row_rect(FILES_SIDEBAR_ITEM_COUNT - 1).bottom() > navigation.rect.bottom()
    {
        return None;
    }
    Some(navigation)
...
pub const fn files_visible_rows_in_viewport(width: u32, height: u32) -> usize {
    if width < FILES_CONTENT_MIN_VIEWPORT_WIDTH {
        0
    } else {
        files_visible_rows(height)
    }
}

pub const fn files_preview_visible_lines(height: u32) -> usize {
    let content_bottom = height.saturating_sub(FILES_PREVIEW_BOTTOM_INSET);
    let first_line_bottom = FILES_PREVIEW_LINE_Y.saturating_add(FILES_PREVIEW_LINE_HEIGHT);
    if content_bottom < first_line_bottom {
        return 0;
    }
    let lines = 1 + (content_bottom - first_line_bottom) / FILES_PREVIEW_LINE_STRIDE;
    if lines as usize > FILES_PREVIEW_VISIBLE_LINES {
...
pub const fn files_preview_visible_lines_in_viewport(width: u32, height: u32) -> usize {
    if width < FILES_CONTENT_MIN_VIEWPORT_WIDTH {
        0
    } else {
...
impl FilesWindowModel {
    ...
    pub fn list_scrollbar_in_viewport(
        ...
        let visible_rows = files_visible_rows_in_viewport(width, height);
        if visible_rows == 0 {
            return None;
        }
    ...
    pub fn preview_scrollbar(&self, width: u32) -> Option<FilesScrollbarLayout> {
        self.preview_scrollbar_in_viewport(width, 420)
    }

    pub fn preview_scrollbar_in_viewport(
        &self,
        width: u32,
        height: u32,
    ) -> Option<FilesScrollbarLayout> {
        ...
        FilesScrollbarLayout::new(
            Rect {
                ...
                width: FILES_SCROLLBAR_WIDTH,
                height: height
                    .saturating_sub(FILES_PREVIEW_BOTTOM_INSET)
                    .saturating_sub(FILES_PREVIEW_SCROLLBAR_Y)
                    .min(FILES_PREVIEW_SCROLLBAR_HEIGHT),
            },
    ...
    pub fn active_scrollbar_in_viewport(
        ...
        self.preview_scrollbar_in_viewport(width, height)
    ...
    pub fn entry_row_in_viewport(
        ...
        if visible_index >= files_visible_rows_in_viewport(width, height) {
            return None;
        }
        ...
        let row_width = width.saturating_sub(FILES_LIST_ROW_WIDTH_INSET);
    ...
    pub fn sidebar_row(&self, height: u32, index: usize) -> Option<ListRow<'_>> {
        self.sidebar_row_in_viewport(640, height, index)
    }

    pub fn sidebar_row_in_viewport(
        &self,
        width: u32,
        height: u32,
        index: usize,
        ...
        let navigation = files_sidebar_navigation_in_viewport(width, height)?;
    ...
    pub fn sidebar_at(&self, height: u32, x: u32, y: u32) -> Option<usize> {
        self.sidebar_at_in_viewport(640, height, x, y)
    }

    pub fn sidebar_at_in_viewport(&self, width: u32, height: u32, x: u32, y: u32) -> Option<usize> {
        (0..self.sidebar_items.len()).find(|index| {
            self.sidebar_row_in_viewport(width, height, *index)
                .is_some_and(|row| row.pointer_hit(x, y))
        })
    }
    ...
    pub fn select_at_in_viewport(
        ...
        if let Some(index) = self.sidebar_at_in_viewport(width, height, x, y) {
...
impl FilesNavigator {
    ...
    pub fn handle_scroll(&mut self, rows: isize) -> FilesNavigation {
        self.handle_scroll_in_viewport(640, 420, rows)
    }

    pub fn handle_scroll_in_viewport(
        &mut self,
        width: u32,
        height: u32,
        rows: isize,
    ...
    pub fn handle_key_in_viewport(
        ...
        if self.window.preview.is_some() {
            return self.handle_preview_key(width, height, key);
        }
        match key {
            FilesKey::Left => {
                ...
                if self
                    .window
                    .sidebar_row_in_viewport(width, height, focused)
                    .is_none()
                {
                    return FilesNavigation::None;
                }
            ...
            FilesKey::Up if self.window.focused_sidebar.is_some() => {
                self.navigate_sidebar(width, height, SidebarNavigationKey::Previous)
            }
            FilesKey::Down if self.window.focused_sidebar.is_some() => {
                self.navigate_sidebar(width, height, SidebarNavigationKey::Next)
            }
            FilesKey::Home if self.window.focused_sidebar.is_some() => {
                self.navigate_sidebar(width, height, SidebarNavigationKey::Home)
            }
            FilesKey::End if self.window.focused_sidebar.is_some() => {
                self.navigate_sidebar(width, height, SidebarNavigationKey::End)
            }
            FilesKey::PageUp | FilesKey::PageDown if self.window.focused_sidebar.is_some() => {
                FilesNavigation::None
            }
            FilesKey::Up => self.navigate_list(width, height, ListNavigationKey::Previous),
            FilesKey::Down => self.navigate_list(width, height, ListNavigationKey::Next),
            FilesKey::PageUp => self.navigate_list(width, height, ListNavigationKey::PagePrevious),
            FilesKey::PageDown => self.navigate_list(width, height, ListNavigationKey::PageNext),
            FilesKey::Home => self.navigate_list(width, height, ListNavigationKey::Home),
            FilesKey::End => self.navigate_list(width, height, ListNavigationKey::End),
            FilesKey::Activate => {
                if let Some(index) = self.window.focused_sidebar {
                    if !self
                        .window
                        .sidebar_row_in_viewport(width, height, index)
    ...
    fn handle_preview_key(&mut self, width: u32, height: u32, key: FilesKey) -> FilesNavigation {
        ...
        match key {
            ...
            FilesKey::PageUp => {
                self.handle_scroll_in_viewport(width, height, -(visible_lines as isize))
            }
            FilesKey::PageDown => {
                self.handle_scroll_in_viewport(width, height, visible_lines as isize)
            }
            FilesKey::Home => self.set_preview_scroll_offset(width, height, 0),
            FilesKey::End => {
                let maximum_offset = self
                    ...
                    .unwrap_or(0);
                self.set_preview_scroll_offset(width, height, maximum_offset)
            }
    ...
    fn navigate_sidebar(
        ...
        let Some(navigation) = files_sidebar_navigation_in_viewport(width, height) else {
            return FilesNavigation::None;
        };
        let Some(focused) =
            navigation.keyboard_target(selected, self.window.sidebar_items.len(), key)
        else {
            return FilesNavigation::None;
        };
        self.window.focused_sidebar = Some(focused);
...
mod tests {
    ...
    fn files_window_model_covers_home_and_empty_locations() {
        ...
        assert_eq!(files_visible_rows_in_viewport(275, 420), 4);
        assert_eq!(files_visible_rows_in_viewport(274, 420), 0);
        ...
        assert!(home.entry_row_in_viewport(274, 420, 0).is_none());
        ...
        assert!(files_sidebar_navigation_in_viewport(172, 350).is_some());
        assert!(files_sidebar_navigation_in_viewport(171, 420).is_none());
        assert!(files_sidebar_navigation_in_viewport(640, 349).is_none());
        assert!(home.sidebar_row_in_viewport(172, 420, 0).is_some());
        assert!(home.sidebar_row_in_viewport(171, 420, 0).is_none());
        assert_eq!(
            home.clone().select_at_in_viewport(171, 420, 40, 134),
            FilesSelection::None
        );
        assert_eq!(
            home.clone().select_at_in_viewport(172, 420, 40, 134),
            FilesSelection::Sidebar(0)
        );
    ...
    fn files_sidebar_keyboard_focus_uses_shared_navigation_before_activation() {
        ...
        assert_eq!(
            navigator.handle_key_in_viewport(171, 420, FilesKey::Left),
            FilesNavigation::None
        );
        assert_eq!(navigator.window().focused_sidebar, None);
        assert_eq!(
        ...
        assert_eq!(
            navigator.handle_key_in_viewport(171, 420, FilesKey::Down),
            FilesNavigation::None
        );
        assert_eq!(navigator.window().focused_sidebar, Some(0));
        assert_eq!(
    ...
    #[test]
    fn files_content_input_uses_visible_rows_from_the_client_viewport() {
        let root = std::env::temp_dir().join(format!(
        ...
        let mut navigator = FilesNavigator::open(&root).expect("navigator should open");
        assert_eq!(
            navigator.handle_key_in_viewport(274, 420, FilesKey::Down),
            FilesNavigation::None
        );
        assert_eq!(navigator.window().selected_entry, None);
        assert_eq!(
            navigator.handle_scroll_in_viewport(274, 420, 1),
            FilesNavigation::None
        );
        assert_eq!(navigator.window().scroll_offset, 0);
        assert!(navigator
            .window()
            .list_scrollbar_in_viewport(274, 300)
            .is_none());
        for expected in 0..=2 {
            assert_eq!(
                navigator.handle_key_in_viewport(640, 420, FilesKey::Down),
        ...
        assert_eq!(navigator.window().scroll_offset, 0);
        assert_eq!(
            navigator.handle_key_in_viewport(274, 420, FilesKey::Activate),
            FilesNavigation::None
        );
        assert_eq!(navigator.current(), canonical_root);
        assert_eq!(
    ...
    fn files_navigator_scrolls_and_previews_only_small_root_confined_text() {
        ...
        assert!(navigator
            .window()
            .preview_scrollbar_in_viewport(274, 420)
            .is_none());
        assert_eq!(
            navigator.handle_scroll_in_viewport(274, 420, 1),
            FilesNavigation::None
        ...
        assert_eq!(
            navigator.handle_key_in_viewport(274, 420, FilesKey::End),
            FilesNavigation::None
        );
        assert_eq!(
        ...
        let compact_preview_scrollbar = navigator
            .window()
            .preview_scrollbar_in_viewport(640, 322)
            .expect("compact Files preview scrollbar");
        assert_eq!(compact_preview_scrollbar.track.height, 38);
        assert_eq!(compact_preview_scrollbar.maximum_offset, 6);
        assert_eq!(compact_preview_scrollbar.thumb.height, 24);
        assert_eq!(
            navigator.handle_key_in_viewport(640, 322, FilesKey::PageDown),
            FilesNavigation::PreviewScrolled
        );
        assert_eq!(
            navigator.window().preview.as_ref().unwrap().scroll_offset,
        ...
        assert_eq!(
            navigator.handle_key_in_viewport(640, 322, FilesKey::Home),
            FilesNavigation::PreviewScrolled
        );
        assert!(navigator
            .window()
            .preview_scrollbar_in_viewport(640, 297)
            .is_none());
        assert_eq!(
            navigator.handle_key_in_viewport(640, 297, FilesKey::End),
            FilesNavigation::None
        );
