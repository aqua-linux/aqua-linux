...
docker run --rm --platform linux/amd64 \
    ...
    sh -eu -c '
        ...
        printf "%s\n" "$output_matrix" | grep -Fq "output_count=4"
        printf "%s\n" "$output_matrix" | grep -Fq "declared_scale_count=4"
        printf "%s\n" "$output_matrix" | grep -Fq "declared_transform_count=4"
        printf "%s\n" "$output_matrix" | grep -Fq "fractional_scales_match=true"
        printf "%s\n" "$output_matrix" | grep -Fq "transforms_match=true"
        ...
        printf "%s\n" "$output_matrix" | grep -Fq "hotplug_add_reaches_both_clients=true"
        ...
        cargo test --quiet -p aqua-compositor --features smithay-smoke --lib \
            smithay_desktop_context_menu_keyboard_is_compositor_owned
        cargo test --quiet -p aqua-compositor --features smithay-smoke --lib \
        ...
        cargo test --quiet -p aqua-compositor --features smithay-smoke --lib \
            first_party_settings_keyboard_uses_shared_control_targets
        cargo test --quiet -p aqua-compositor --features smithay-smoke --lib \
