...
CONTROL_POLICY_RESTART_EXHAUSTION_RUNNER="${ROOT_DIR}/scripts/check-audio-control-policy-restart-exhaustion-qemu.sh"
...
test -x "${CONTROL_SUBMISSION_BUDGET_RUNNER}"
grep -Fq 'AQUA_AUDIO_QEMU_CONTRACT=control-submission-budget' \
    "${CONTROL_SUBMISSION_BUDGET_RUNNER}"
for contract in \
    'aqua-audio-adapter-probe' \
    'failed_submissions=3 fourth_submission_blocked=true' \
    'generation_stable=true recovery_generation_advanced=true' \
    'authoritative_ack=true services_running=true recovery_shell=true'
do
    grep -Fq -- "${contract}" "${RUNNER}" "${EXPECT_SCRIPT}" \
        "${CONTROL_SUBMISSION_BUDGET_RUNNER}"
done
...
test -x "${CONTROL_RESTART_EXHAUSTION_RUNNER}"
grep -Fq 'AQUA_AUDIO_QEMU_CONTRACT=control-restart-exhaustion' \
    "${CONTROL_RESTART_EXHAUSTION_RUNNER}"
for contract in \
    'aqua-audio-probe controls' \
    'status=degraded reason=restart-limit failed_service=pipewire attempts=4 restarts=3' \
    'control_before=true control_blocked=true false_acknowledgement=false' \
    'full_stack_retired=true services_stopped=true' \
    'max_restarts=3' \
    'media-service-supervisor.pid' \
    'pipewire-0'
do
...
test -x "${CONTROL_POLICY_RESTART_EXHAUSTION_RUNNER}"
grep -Fq 'AQUA_AUDIO_QEMU_CONTRACT=control-policy-restart-exhaustion' \
    "${CONTROL_POLICY_RESTART_EXHAUSTION_RUNNER}"
for contract in \
    'aqua-audio-probe controls' \
    'status=degraded reason=restart-limit failed_service=wireplumber attempts=4 restarts=3' \
    'control_before=true control_blocked=true false_acknowledgement=false' \
