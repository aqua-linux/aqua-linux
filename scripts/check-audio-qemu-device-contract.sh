...
test -x "${CONTROL_ROUTE_LOSS_RUNNER}"
grep -Fq 'AQUA_AUDIO_QEMU_CONTRACT=control-route-loss' \
    "${CONTROL_ROUTE_LOSS_RUNNER}"
for contract in \
    'aqua-audio-adapter-probe route-generation-loss' \
    'target_bound=true selected_route=true fallback_prepared=true' \
    'old_request_confirmed=false request_rejected_or_lost=true' \
    'fallback_matches_desired=true no_resubmission=true' \
    'removed_default=true false_acknowledgement=false' \
