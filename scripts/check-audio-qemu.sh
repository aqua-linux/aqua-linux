...
if test "${AQUA_AUDIO_QEMU_CONTRACT}" = output; then
    ...
    echo "Playback capture: ${WAV_CAPTURE}"
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = policy-service-loss; then
    for marker in \
        '[AQUA-AUDIO] stage=qemu-device status=ok driver=snd_hda_intel codec=hda-duplex playback=true capture_node=true' \
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-AUDIO] stage=qemu-policy-loss-precondition status=ok active_stream=true frames=480 pipewire_pid=true wireplumber_pid=true' \
        '[AQUA-AUDIO] stage=media-probe status=interrupted direction=playback reason=pcm-io' \
        '[AQUA-AUDIO] stage=qemu-policy-service-loss status=ok failed_service=wireplumber active_stream_aborted=true false_success=false full_stack_restart=true old_processes_retired=true restart_recovery=true playback_after=true recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
    ...
    test "$(grep -Fc '[AQUA-AUDIO] stage=media-probe status=ok direction=playback frames=48000 rate=48000 channels=2 format=s16le' "${SERIAL_LOG}")" -eq 1
    python3 "${ROOT_DIR}/scripts/check-qemu-audio-wave.py" "${WAV_CAPTURE}"
    echo 'Aqua Linux active audio policy-service loss recovery check passed.'
    echo "Playback capture: ${WAV_CAPTURE}"
...
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = control-route-loss; then
    for marker in \
        ...
        '[AQUA-AUDIO] stage=qemu-control-route-loss status=ok target_bound=true removed_default=true false_acknowledgement=false request_rejected_or_lost=true fallback_matches_desired=true no_resubmission=true services_running=true recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
...
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = control-policy-service-loss; then
    for marker in \
        '[AQUA-AUDIO] stage=qemu-device status=ok driver=snd_hda_intel codec=hda-duplex playback=true capture_node=true' \
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-MEDIA] stage=media-service-supervisor status=restarting failed_service=wireplumber' \
        '[AQUA-AUDIO] stage=control-probe status=failed operation=open' \
        '[AQUA-AUDIO] stage=qemu-control-policy-service-loss status=ok failed_service=wireplumber control_rejected=true false_acknowledgement=false full_stack_restart=true old_processes_retired=true restart_recovery=true control_after=true recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
    test "$(grep -Fc '[AQUA-AUDIO] stage=control-probe status=ok backend=aqua-audio-native default_sink=true volume=35 mute_cycle=true' "${SERIAL_LOG}")" -eq 2
    ...
    echo 'Aqua Linux audio control policy-service loss acknowledgement check passed.'
    ...
    for marker in \
        ...
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-MEDIA] stage=media-service-supervisor status=degraded reason=restart-limit failed_service=pipewire attempts=4 restarts=3' \
        '[AQUA-AUDIO] stage=qemu-capture-restart-exhaustion status=ok failed_service=pipewire attempts=4 restarts=3 restart_limit=true full_stack_retired=true services_stopped=true capture_before=true capture_blocked=true false_success=false controlled_pattern=zero-pcm host_microphone=false recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
    test "$(grep -Fc '[AQUA-MEDIA] stage=media-service-supervisor status=restarting failed_service=pipewire' "${SERIAL_LOG}")" -eq 3
    test "$(grep -Fc '[AQUA-AUDIO] stage=media-probe status=ok direction=capture frames=4800 rate=48000 channels=2 format=s16le peak_abs=0 pattern=silence' "${SERIAL_LOG}")" -eq 1
    echo 'Aqua Linux QEMU audio capture restart-exhaustion check passed.'
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = policy-restart-exhaustion; then
    for marker in \
        '[AQUA-AUDIO] stage=qemu-device status=ok driver=snd_hda_intel codec=hda-duplex playback=true capture_node=true' \
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-MEDIA] stage=media-service-supervisor status=degraded reason=restart-limit failed_service=wireplumber attempts=4 restarts=3' \
        '[AQUA-AUDIO] stage=qemu-policy-restart-exhaustion status=ok failed_service=wireplumber attempts=4 restarts=3 restart_limit=true full_stack_retired=true services_stopped=true playback_blocked=true recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
    test "$(grep -Fc '[AQUA-MEDIA] stage=media-service-supervisor status=restarting failed_service=wireplumber' "${SERIAL_LOG}")" -eq 3
    echo 'Aqua Linux QEMU audio policy-service restart-exhaustion check passed.'
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = control-policy-restart-exhaustion; then
    for marker in \
        '[AQUA-AUDIO] stage=qemu-device status=ok driver=snd_hda_intel codec=hda-duplex playback=true capture_node=true' \
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-MEDIA] stage=media-service-supervisor status=degraded reason=restart-limit failed_service=wireplumber attempts=4 restarts=3' \
        '[AQUA-AUDIO] stage=qemu-control-policy-restart-exhaustion status=ok failed_service=wireplumber attempts=4 restarts=3 restart_limit=true full_stack_retired=true services_stopped=true control_before=true control_blocked=true false_acknowledgement=false recovery_shell=true'
    do
        grep -Fq "${marker}" "${SERIAL_LOG}"
    done
    test "$(grep -Fc '[AQUA-MEDIA] stage=media-service-supervisor status=restarting failed_service=wireplumber' "${SERIAL_LOG}")" -eq 3
    test "$(grep -Fc '[AQUA-AUDIO] stage=control-probe status=ok backend=aqua-audio-native default_sink=true volume=35 mute_cycle=true' "${SERIAL_LOG}")" -eq 1
    test "$(grep -Fc '[AQUA-AUDIO] stage=control-probe status=failed operation=open code=-2 detail=Unable to connect to PipeWire' "${SERIAL_LOG}")" -eq 1
    echo 'Aqua Linux QEMU audio control policy-service restart-exhaustion check passed.'
elif test "${AQUA_AUDIO_QEMU_CONTRACT}" = capture-policy-restart-exhaustion; then
    for marker in \
        '[AQUA-AUDIO] stage=qemu-device status=ok driver=snd_hda_intel codec=hda-duplex playback=true capture_node=true' \
        '[AQUA-AUDIO] stage=qemu-service status=ok owner_uid=1000 pipewire=true wireplumber=true sink=true source=true' \
        '[AQUA-MEDIA] stage=media-service-supervisor status=degraded reason=restart-limit failed_service=wireplumber attempts=4 restarts=3' \
    ...
    test "$(grep -Fc '[AQUA-MEDIA] stage=media-service-supervisor status=restarting failed_service=wireplumber' "${SERIAL_LOG}")" -eq 3
    test "$(grep -Fc '[AQUA-AUDIO] stage=media-probe status=ok direction=capture frames=4800 rate=48000 channels=2 format=s16le peak_abs=0 pattern=silence' "${SERIAL_LOG}")" -eq 1
    echo 'Aqua Linux QEMU audio capture policy-service restart-exhaustion check passed.'
