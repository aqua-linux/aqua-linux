...
grep -Fq '[AQUA-WIFI] stage=qemu-hwsim-acceptance status=ok radios=2 discovery=true rescan=true transition_mode=true wpa3=true wpa3_session_only=true new_credential=true association=true dhcp=true disconnect=true reconnect=true service_recovery=true forget=true disable=true broker_auth=true default_wifi=false recovery_shell=true' "$SERIAL_LOG"
